(function() {
	//todo: fix this! references Array, rather than making a copy of it
	var $$ = Array;
	
	if(!$$.prototype.filter) {
		$$.prototype.filter = function(fn, thisp) {
			var arr = new $$();
			for(var i=0, l=this.length; i<l; ++i)
				if(fn.call(thisp, this[i], i, this)) arr.push(this[i]);
			return arr;
		};
	}
	if(!$$.prototype.forEach) {
		$$.prototype.forEach = function(fn, thisp) {
			for(var i=0, l=this.length; i<l; ++i)
				fn.call(thisp, this[i], i, this);
		};
	}
	if(!$$.prototype.indexOf) {
		$$.prototype.indexOf = function(e, i) {
			if(i==undefined) i = 0;
			for(var l=this.length; i<l; ++i) if(this[i]==e) return i;
			return -1;
		};
	}
	if(!$$.prototype.lastIndexOf) {
		$$.prototype.lastIndexOf = function(e, i) {
			if(i==undefined) i = this.length;
			while(i--) if(this[i]==e) return i;
			return -1;
		};
	}
	if(!$$.prototype.map) {
		$$.prototype.map = function(fn, thisp) {
			var arr = new $$();
			for(var i=0, l=this.length; i<l; ++i)
				arr.push(fn.call(thisp, this[i], i, this));
			return arr;
		};
	}
	if(!$$.prototype.some) {
		$$.prototype.some = function(fn, thisp) {
			for(var i=0, l=this.length; i<l; ++i)
	...
	if(document.documentElement.compareDocumentPosition) {
		compare = function(n1,n2) {
			if(n1.compareDocumentPosition(n2) & 4) return -1;
			else return n1==n2?0:1;
		};
	...
	$$.prototype.merge = function(arr) {
		...
		var j=0, lj=arr.length;
		for(var s, i=0, li=this.length; i<li && j<lj; ++i)
			if((s=compare(this[i], arr[j]))==0) ++j;
			else if(s>0) this.splice(i,0, arr[j++]), ++li;
		if(j<lj) this.concat2(arr.slice(j));
	};
	
	$$.prototype.contains = function(e) {
		return this.lastIndexOf(e)!=-1;
	};
	...
	var getByClass = ('getElementsByClassName' in tester)?
		...
		function(e,c) {
			...
			for(var elems=e.getElementsByTagName('*'), i=0, l=elems.length;
																	i<l; ++i)
				if(matcher.test(elems[i].className)) arr.push(elems[i]);
			return arr;
		};
	...
	//~ if(document.querySelectorAll) {
		...
		var nth = function(fn,found) {
			...
			if(p!=-1) c1 = (p && fn[p-1]=='-')?-1:((fn.substr(0,p)-0) || 1);
			else c1 = 0;
			if(c0<0 && c1<=0) {
				return new $$();
			} else if(c1==0) {
				...
				if(c0) return (c0>0)?found.slice(c0-1):found.slice(0,-c0);
				else return found;
			}
			var elems = new $$();
			var cache1 = new $$();
			...
			for(var k,ee0,cc0,j=0,lj=found.length; j<lj; ++j) {
				...
				if((cc0-c0)%c1==0 && (cc0-c0)/c1>=0) elems.push(found[j]);
				if(k!=-1) {
					cache2[k] = found[j];
					cache3[k] = cc0;
				} else {
		...
		var select = function(selector, parents) {
			if(!parents) parents = new $$(document);
			else if(!parents.push) parents = new $$(parents);
			...
			for(var i=0, parts=selector.split(r.white), l=parts.length;
				...
				if(found!=-1) {
					...
					//todo: contains should be called only on first pass?
						...
						for(var childs,j=0,lj=parents.length; j<lj;) {
							childs=parents[j++];
							if(elems.contains(childs)) continue;
							while(childs=nextSibling(childs))
								elems.push(childs);
						}
					}
		...
		var select2 = function(selector,parents,found) {
			...
			if(check=selector.match(r.tag)) {
				if(!found.length) {
					...
					for(var k=0, lk=found.length; k<lk; ++k)
						if(check==found[k].tagName) elems.push(found[k]);
					found = elems;
				}
			...
			if(check=selector.match(r.attr)) {
				if(!found.length &&
								!(found=getter(parents, getByTags, '*')).length)
					return found;
				var checker = [];
			...
			if(check=selector.match(r.colon)) {
				...
				for(var checks=check, q=0, lq=checks.length; q<lq; ++q) {
					...
					if(pseudo=='button') {
						...
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].type=='checkbox') elems.push(found[j]);
					} else if(pseudo=='checked') {
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].checked) elems.push(found[j]);
						...
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].disabled) elems.push(found[j]);
					} else if(pseudo=='empty') {
						for(var j=0,lj=found.length; j<lj; ++j)
							if(!found[j].firstChild) elems.push(found[j]);
					} else if(pseudo=='enabled') {
						for(var j=0,lj=found.length; j<lj; ++j)
							if(!found[j].disabled && found[j].type!='hidden')
								elems.push(found[j]);
					} else if(pseudo=='eq') {
						if((check=val-0)<found.length) elems.push(found[check]);
						else return new $$();
					} else if(pseudo=='even' ||
						...
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].type=='file') elems.push(found[j]);
					} else if(pseudo=='first-child') {
						var cache;
						for(var j=0,lj=found.length; j<lj; ++j) {
							if(cache==found[j].parentNode) continue;
							cache = found[j].parentNode;
							if(found[j]==firstChild(cache))
								elems.push(found[j]);
						}
					} else if(pseudo=='first') {
						if(found.length) elems.push(found[0]);
						else return found;
					} else if(pseudo=='gt') {
						...
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].type=='image') elems.push(found[j]);
						...
						if(found.length) elems.push(found[found.length-1]);
						else return found;
					} else if(pseudo=='lt') {
						...
						for(var j=0, lj=found.length; j<lj; ++j)
							if(!check.contains(found[j])) elems.push(found[j]);
					} else if(pseudo=='odd' ||
						...
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].type=='password') elems.push(found[j]);
					} else if(pseudo=='radio') {
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].type=='radio') elems.push(found[j]);
					} else if(pseudo=='reset') {
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].type=='reset') elems.push(found[j]);
					} else if(pseudo=='selected') {
						for(var j=0,lj=found.length; j<lj; ++j) {
							found[j].parentNode.selectedIndex;
							if(found[j].selected===true) elems.push(found[j]);
						}
					} else if(pseudo=='submit') {
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].type=='submit') elems.push(found[j]);
					} else if(pseudo=='text') {
						for(var j=0,lj=found.length; j<lj; ++j)
							if(found[j].type=='text') elems.push(found[j]);
						...
						for(var fChild,j=0,lj=found.length; j<lj; ++j) {
							...
							if(!fChild) elems.push(found[j]);
						}
					}
					if(elems.length) found = elems;
					else return elems;
				}
			}
	...
	$$.prototype.children = function(selector) {
		...
		for(var childs,i=0,l=this.length; i<l;)
			if(childs=firstChild(parents[i++]))
				do elems.push(childs);
				while(childs=nextSibling(childs));
		if(selector) elems = select2(selector,this,elems);
		return elems;
	};
	/*
	var ANIMRATE = 30;
